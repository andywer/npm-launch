...
function createListr (tasks, taskCalls) {
  const taskList = Object.keys(tasks).map((taskName) =>
    ({
      title: taskName,
      task: () => createListrTask(tasks, taskName)
    })
  )
...
function createListrTask (tasks, taskName) {
  const task = tasks[ taskName ]

  if (typeof task === 'function') {
    return createListrTaskByPromise(task)
...
function createListrTaskByPromise (method) {
